...
letters.CART <- rpart(letters.formula, data = lettersTrain, method = "class", control = rpart.control(minbucket = 25))

plot(letters.CART)
text(letters.CART, pretty = 0)

letters.CARTpredTest <- predict(letters.CART, newdata = lettersTest, type = "class")

letters.CARTpredTable <- table(lettersTest$Letter, letters.CARTpredTest)
letters.CARTpredTable

sum(diag(letters.CARTpredTable))/nrow(lettersTest)

# 2) Do the same as above for random forest. Use nodesize
#    = 25 and ntree = 200.

letters.forest <- randomForest(letters.formula, data = lettersTrain, nodesize = 25, ntree = 200)

letters.forestPredTest <- predict(letters.forest, newdata = lettersTest)
letters.forestPredTable <- table(lettersTest$Letter, letters.forestPredTest)
letters.forestPredTable

sum(diag(letters.forestPredTable))/nrow(lettersTest)
