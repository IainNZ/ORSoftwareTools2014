...
str(letters)
LettersDist = dist(letters[2:17], method = "euclidean")
LettersHC = hclust(LettersDist, method = "ward")

#  b) Plot the dendrogram and use it to 
#     decide how many clusters to select.

plot(LettersHC)
rect.hclust(LettersHC, k = 4, border = "red")
LettersHCGroups = cutree(LettersHC, k = 4)

#  c) Make a table comparing the "Letter" 
#     attribute compared with the HC assignment

table(letters$Letter, LettersHCGroups)

# 2) Do the same using k-means clustering. 
#    How well do you think clustering performs
#    on this dataset?

LettersKMC = kmeans(letters[2:17], centers = 4, iter.max = 100)
LettersKMCGroups = LettersKMC$cluster
table(letters$Letter, LettersKMC$cluster)
...
LettersKMC = kmeans(letters[2:17], centers = 4, iter.max = 100, nstart = 1000)
LettersKMCGroups = LettersKMC$cluster
table(letters$Letter, LettersKMC$cluster)
