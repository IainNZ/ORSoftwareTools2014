...
hist(CEOcomp$Years, xlab = "Years", main = "Histogram of Number of Years of Experience for CEOs")

##
# 3a) Load the Hubway dataset "stations.csv"

bikeStations <- read.csv("stations.csv")
...
mean(bikeStations$lng)
mean(bikeStations$lat)
...
plot(bikeStations$lng, bikeStations$lat, main = "Map of Stations", xlab = "Longitude (W - E)", ylab = "Latitude (S - N)")
...
?round
round(2.52, digits = 2) # test functionality of round

bikeStations$lat2 <- round(bikeStations$lat, digits = 2)
bikeStations$lng2 <- round(bikeStations$lng, digits = 2)

table(bikeStations$lat2, bikeStations$lng2)
