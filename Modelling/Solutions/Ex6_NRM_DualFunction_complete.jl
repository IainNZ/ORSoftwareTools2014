...
function solveNRMProblem( legs, fareLegs, fareProbabilities, fareRevenues, 
	...
	# capacity constraints - create constraint references:
	@defConstrRef legCapConstr[1:numLegs]

	for ell = 1:numLegs
		legCapConstr[ell] = @addConstraint(m, sum{ x[f] , f = 1:numFares; ell in fareLegs[f]} <= legCapacities[ell])
	end
	...
	dualVec = zeros(numLegs,1)
	for ell = 1:numLegs
		dualVec[ell] = getDual( legCapConstr[ell] )
	end
	
	# Step 7: return the vector of dual variables.
	return dualVec, getObjectiveValue(m)
