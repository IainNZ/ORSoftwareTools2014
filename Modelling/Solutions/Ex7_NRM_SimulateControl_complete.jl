	...
	for i = 1:numSims
		...
		for t=1:T
			...
			if (request != numFares+1 
				...
				dualPrices, obj = solveNRMProblem( legs, fareLegs, fareProbabilities, fareRevenues, legCapacities - usedCap[:], T-t+1)				
				# ...and check if the revenue from the request exceeds the 
				# opportunity cost (~= the sum of the dual prices of the consumed legs) 
				if( sum([dualPrices[ell] for ell in fareLegs[request]]) < fareRevenues[request])
					# Accept the request!
